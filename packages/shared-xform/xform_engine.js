...
async function transform(spec, input, vars = {}, options = {}) {
    ...
    if (spec.filters && spec.filters.length > 0) {
        rows = rows.filter(row => {
            return spec.filters.every(f => {
                try {
                    const expr = (0, xform_vars_1.interpolateExpr)(f.expr, vars, inputType === 'json' ? 'jmespath' : inputType === 'xml' ? 'xpath' : 'css');
                    if (inputType === 'json')
    ...
    for (const row of rows) {
        ...
        for (const m of spec.mappings) {
            try {
                const expr = (0, xform_vars_1.interpolateExpr)(m.expr, vars, inputType === 'json' ? 'jmespath' : inputType === 'xml' ? 'xpath' : 'css');
                let val;
                if (inputType === 'json')
    ...
    if (spec.output.type === 'csv') {
        ...
        return (0, xform_renderers_1.renderText)(records.map(r => String(r[col])));
    }
